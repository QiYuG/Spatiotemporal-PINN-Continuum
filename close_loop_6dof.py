...
# 任务分配表: (GPU_ID, 进度条行号, 模型名称)
# 巧妙调度：GPU 0 先秒算 PID，接着算 Residual_condition
CTRL_TASKS = [
    (0, 0, "PID_Only"),
    (0, 1, "CTC_Residual_condition"),
    (1, 2, "CTC_Residual_margin"),
    (2, 3, "CTC_Tau_margin"),
    (3, 4, "CTC_Tau_condition") 
]
...
def evaluate_controller(args):
    ...
    desc_str = f"[GPU {gpu_id}] {ctrl_name[:22]:<22}"
    pbar = tqdm(range(TOTAL_STEPS), desc=desc_str, position=pos_id, leave=True)
    ...
    # 释放显存，保证多个任务共用一个GPU时的显存绝对安全 (尤其是 6-DOF 模型占用更大)
...
if __name__ == "__main__":
    ...
    plt.savefig("Adaptive_Control_Tracking_6DOF_All.png", dpi=300)
    ...
    plt.tight_layout()
    plt.savefig("Adaptive_Control_Effort_6DOF_All.png", dpi=300)
    
    print("✅ 6-DOF 闭环自适应控制实验图表已保存为 Adaptive_Control_Tracking_6DOF_All.png 和 Adaptive_Control_Effort_6DOF_All.png")
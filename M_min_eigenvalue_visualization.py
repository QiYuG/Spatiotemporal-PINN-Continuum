...
if __name__ == "__main__":
    ...
    # 分配 4 个模型到 4 张显卡
    # tasks = [
    #     (0, "Residual Margin", PINN_Residual, f"models/final_Residual_margin_{DATASET_TYPE}.pth"),
    #     (1, "Residual Condition", PINN_Residual, f"models/final_Residual_condition_{DATASET_TYPE}.pth"),
    #     (2, "Tau Margin", PINN_Tau, f"models/final_Tau_margin_{DATASET_TYPE}.pth"),
    #     (3, "Tau Condition", PINN_Tau, f"models/final_Tau_condition_{DATASET_TYPE}.pth")
    # ]
    tasks = [
        (0, "Residual Margin", PINN_Residual, f"models_6dof_evo/final_evo_Residual_margin_{DATASET_TYPE}_new.pth"),
        (1, "Residual Condition", PINN_Residual, f"models_6dof_evo/final_evo_Residual_condition_{DATASET_TYPE}_new.pth"),
        (2, "Tau Margin", PINN_Tau, f"models_6dof_evo/final_evo_Tau_margin_{DATASET_TYPE}_new.pth"),
        (3, "Tau Condition", PINN_Tau, f"models_6dof_evo/final_evo_Tau_condition_{DATASET_TYPE}_new.pth")
    ]
    ...
    plot_order = ["Residual Margin", "Residual Condition", "Tau Margin", "Tau Condition"]
    ...
    plt.tight_layout()
    plt.savefig("min_eig_results/MinEigenvalue_Spatial_Distribution_4GPU_6dof_evo_new.png", dpi=300, bbox_inches='tight')

    # 保存矩阵数据
    for name in plot_order:
        safe_name = name.replace(" ", "_")
        np.save(f"min_eig_results/min_eig_{safe_name}_grid_6dof_evo_new.npy", results_dict[name])

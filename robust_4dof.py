...
def evaluate_model(args):
    ...
    if is_evo:
        path = f"{EVO_DIR}/final_evo_{model_name}_{MODEL_TYPE}.pth"
    else:
        path = f"{NO_EVO_DIR}/final_{model_name}_{MODEL_TYPE}.pth"
        if not os.path.exists(path):
            path = f"{NO_EVO_DIR}/best_{model_name}_{MODEL_TYPE}.pth" # Fallback
